...
class Allweather(data.Dataset):
    ...
    def _load_clear_image(self, haze_path):
        try:
            # 从haze_path构建clear_path
            filename = os.path.basename(haze_path)
            clear_path = os.path.join(self.clear_dir, filename)
            
            # 检查文件是否存在
            if not os.path.exists(clear_path):
                print(f"Clear image not found: {clear_path}, using a placeholder instead")
                # 返回一个黑色图像作为占位符
            ...
            if clear_img is None:
                print(f"Error loading clear image: {filename}, {haze_path}")
                # 返回一个黑色图像作为占位符
                placeholder = np.zeros((256, 256, 3), dtype=np.uint8)
                return Image.fromarray(placeholder)
            ...
            print(f"Exception loading clear image {haze_path}: {str(e)}")
            # 返回一个黑色图像作为占位符
            placeholder = np.zeros((256, 256, 3), dtype=np.uint8)
            return Image.fromarray(placeholder)
